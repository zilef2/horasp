<script setup>
    ...
    import Create from '@/Pages/centrocompras/Create.vue';
    import Edit from '@/Pages/centrocompras/Edit.vue';
    import Delete from '@/Pages/centrocompras/Delete.vue';
    import DeleteBulk from '@/Pages/centrocompras/DeleteBulk.vue';
    ...
    watch(() => _.cloneDeep(data.params), debounce(() => {
        ...
        router.get(route("centrocompes.index"), params, {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        })
    ...
    const selectAll = (event) => {
        if (event.target.checked === false) {
            ...
            props.centrocompras?.data.forEach((centrocomp) => {
                data.selectedId.push(centrocomp.id)
            })
        }
    }
    const select = () => {
        if (props.centrocompras?.data.length == data.selectedId.length) {
        ...
        <div class="space-y-4">
            ...
            <div class="relative bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                    ...
                    <table class="w-full">
                        <thead class="uppercase text-sm border-t border-gray-200 dark:border-gray-700">
                            <tr class="dark:bg-gray-900/50 text-left">
                                ...
                                <th class="px-2 py-4 text-center hover:bg-sky-200">#</th>
                                <th v-for="(titulos,indiceN) in nombresTabla[0]" :key="indiceN" v-on:click="order( nombresTabla[1][indiceN] )"
                                        class="px-2 py-4 cursor-pointer hover:bg-sky-200">
                                     <div class="flex justify-between items-center">
                                        <span>{{ titulos }}</span> <ChevronUpDownIcon class="w-4 h-4" />
                                    </div> 
                                </th>
                        ...
                        <tbody>
                            <tr v-for="(clasegenerica, index) in centrocompras.data" :key="index"
                                    ...
                                    <div class="flex justify-center items-center">
                                        <div class="rounded-md overflow-hidden">
                                            <InfoButton type="button"
                                                @click="(data.editOpen = true), (data.clasegenerica = clasegenerica)"
                ...
                <div class="flex justify-between items-center p-2 border-t border-gray-200 dark:border-gray-700">
                    <Pagination :links="props.centrocompras" :filters="data.params" />
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
